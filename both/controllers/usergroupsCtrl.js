UsergroupsController = AppController.extend({
  waitOn: function() {
    return this.subscribe('allUserData');
    //return;
  },
  onAfterAction: function () {
    Meta.setTitle('User groups');
  }
});

UsergroupsController.events({
  'click [data-action=doSomething]': function (event, template) {
    event.preventDefault();
  }
});

usergroupsCollection = new Mongo.Collection("m3coreusergroups") ;

usergroupsCollection.allow({
...
usergroupsCollection.attachSchema(new SimpleSchema({
  name: {
    type: String,
    label: "Name",
    max: 20
...
TabularTables.Usergroups = new Tabular.Table({
  name: "Usergroups",
